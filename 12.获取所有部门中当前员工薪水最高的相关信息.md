...
```mysql
select d.dept_no, s.emp_no , MAX(s.salary) as max_salary  from dept_emp d left join salaries s on d.emp_no = s.emp_no group by d.dept_no, s.emp_no having MAX(s.salary) = max_salary
